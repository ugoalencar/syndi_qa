...
// zoom sem limite de largura e qualidade 100: so entra em jogo quando a foto tem rotacao
// salva (server.js serve o arquivo original direto pro caso comum, sem rotacao) - reencodar
// pra girar e inevitavel, mas mantem o mais proximo possivel do original.
const TAMANHOS = {
    mini: { largura: 500, qualidade: 78 },
    zoom: { largura: null, qualidade: 100 }
};
...
async function gerarPreview(caminhoOriginal, tamanho, rotacao = 0) {
    ...
    if (config.largura) {
        transform = transform.resize({ width: config.largura, withoutEnlargement: true });
    }
    await transform
