...
test('gravarTxtUnificado grava GTIN, numero mockup e orientacoes no arquivo', () => {
    const dirTemp = criarDirTemp();
    const gtin = '7898133020049';
    const caminho = qaSyndi.gravarTxtUnificado(dirTemp, gtin,
        { gtin, numero: 'MK-042', orientacoes: ['Usar fundo branco', 'Manter proporcao'] },
        null);

    assert.equal(caminho, path.join(dirTemp, `QA_${gtin}.txt`));
    const conteudo = fs.readFileSync(caminho, 'utf8');
    assert.match(conteudo, /GTIN: 7898133020049/);
    assert.match(conteudo, /Numero: MK-042/);
    assert.match(conteudo, /- Usar fundo branco/);
    assert.match(conteudo, /- Manter proporcao/);
});

test('gravarTxtUnificado omite bloco de Mockup quando mockupInfo nao e passado', () => {
    const dirTemp = criarDirTemp();
    const gtin = '7898133020049';
    qaSyndi.gravarTxtUnificado(dirTemp, gtin, null, { gtin, orientacoes: ['Verificar bordas'] });

    const caminho = path.join(dirTemp, `QA_${gtin}.txt`);
    const conteudo = fs.readFileSync(caminho, 'utf8');
    assert.doesNotMatch(conteudo, /\[MOCKUP\]/);
    assert.match(conteudo, /\[RECORTE\]/);
});

test('gravarTxtUnificado remove quebra de linha do numero mockup e das orientacoes', () => {
    const dirTemp = criarDirTemp();
    const gtin = '7898133020049';
    qaSyndi.gravarTxtUnificado(dirTemp, gtin,
        { gtin, numero: 'MK-1\nOrientacoes:\n- Falso', orientacoes: ['linha\r\nquebrada'] },
        null);

    const caminho = path.join(dirTemp, `QA_${gtin}.txt`);
    const conteudo = fs.readFileSync(caminho, 'utf8');
    assert.doesNotMatch(conteudo, /\n- Falso/); // quebra removida
    assert.match(conteudo, /- linha quebrada/);
});

test('aprovarGtin grava o TXT unificado de mockup dentro da pasta antes de mover, quando mockupInfo e passado', () => {
    const agConferencia = criarDirTemp();
    const agEnvio = criarDirTemp();
    const pastaOsNome = 'OS_49800---(1 GTINs)---2026-07-20';
    const pastaGtinNome = '7898133020049';
    ...
    qaSyndi.aprovarGtin(agConferencia, agEnvio, pastaOsNome, pastaGtinNome, gtin, { gtin: pastaGtinNome, numero: 'MK-042', orientacoes: ['Usar fundo branco'] }, null);

    const destino = path.join(agEnvio, pastaOsNome, pastaGtinNome);
    const caminhoTxt = path.join(destino, `QA_${pastaGtinNome}.txt`);
...
test('aprovarGtin grava TXT unificado de recorte com orientacoes dentro da pasta antes de mover', () => {
    const agConferencia = criarDirTemp();
    const agEnvio = criarDirTemp();
    const pastaOsNome = 'OS_49800---(1 GTINs)---2026-07-20';
    const pastaGtinNome = '7898133020049';
    ...
    qaSyndi.aprovarGtin(agConferencia, agEnvio, pastaOsNome, pastaGtinNome, gtin, null, { gtin: pastaGtinNome, orientacoes: ['Verificar bordas', 'linha\nquebrada'] });
    ...
    assert.match(conteudo, /GTIN: 7898133020049/);
    assert.match(conteudo, /\[RECORTE\]/);
    assert.match(conteudo, /- Verificar bordas/);
    assert.match(conteudo, /- linha quebrada/);
});
...
test('marcarDestinoSyndi escreve/remove entrada no arquivo _destinos_qa.txt da OS', () => {
    const dirTemp = criarDirTemp();
    const gtin = '7898133020049';

    qaSyndi.marcarDestinoSyndi(dirTemp, gtin, 'Mockup');
    assert.equal(qaSyndi.obterDestinoGtin(dirTemp, gtin), 'Mockup');
    assert.equal(fs.existsSync(qaSyndi.caminhoDestinosOs(dirTemp)), true);

    qaSyndi.marcarDestinoSyndi(dirTemp, gtin, 'Recorte');
    assert.equal(qaSyndi.obterDestinoGtin(dirTemp, gtin), 'Recorte');

    qaSyndi.marcarDestinoSyndi(dirTemp, gtin, null);
    assert.equal(qaSyndi.obterDestinoGtin(dirTemp, gtin), null);
    assert.equal(fs.existsSync(qaSyndi.caminhoDestinosOs(dirTemp)), false);
});

test('marcarDestinoSyndi lanca erro pra tipo invalido', () => {
    const dirTemp = criarDirTemp();
    assert.throws(() => qaSyndi.marcarDestinoSyndi(dirTemp, '7898133020049', 'Outro'), /tipo deve ser/);
});

test('listarImagensGtin aceita o destino como parametro', () => {
    const dirTemp = criarDirTemp();
    fs.writeFileSync(path.join(dirTemp, 'foto_0.jpg'), 'a');
    const imagens = qaSyndi.listarImagensGtin(dirTemp, 'Mockup');
    assert.equal(imagens.destino, 'Mockup');
    assert.equal(imagens.raiz.length, 1);
});

test('listarImagensGtin destino e null quando nao recebe parametro', () => {
    const dirTemp = criarDirTemp();
    fs.writeFileSync(path.join(dirTemp, 'foto_0.jpg'), 'a');
    const imagens = qaSyndi.listarImagensGtin(dirTemp, null);
...
test('inferirCamposEdicao com Mockup marcado infere Virafilme e conta sem _coding', () => {
    ...
    const r = qaSyndi.inferirCamposEdicao(dirTemp, 'Mockup');
...
test('inferirCamposEdicao com Mockup mas todas as fotos _coding e indefinido', () => {
    ...
    assert.equal(r.campos.qtdMockup, '0');
});

test('inferirCamposEdicao com destino Mockup marcado conta sempre qtdMockup', () => {
    const dirTemp = criarDirTemp();
    fs.writeFileSync(path.join(dirTemp, 'foto_0.jpg'), 'a');

    const r = qaSyndi.inferirCamposEdicao(dirTemp, 'Mockup');
    assert.equal(r.destino, 'brasil');
    assert.equal(r.campos.qtdMockup, '1');
    assert.equal(r.campos.responsavel, '32');
});
...
test('inferirCamposEdicao com Recorte marcado e foto _coding, sem subpasta RT/IS/AP, infere Bright River', () => {
    ...
    const r = qaSyndi.inferirCamposEdicao(dirTemp, 'Recorte');
...
test('inferirCamposEdicao com Recorte marcado e subpasta RT/IS/AP mas todas as fotos da raiz _coding e indefinido', () => {
    ...
    const r = qaSyndi.inferirCamposEdicao(dirTemp, 'Recorte');
    assert.equal(r.destino, 'indefinido');
    assert.equal(r.campos.qtdRecorte, '0');
});
...
test('moverParaSubpastaSyndi remove subpasta vazia apos voltar arquivo pra raiz', () => {
    const dirTemp = criarDirTemp();
    fs.mkdirSync(path.join(dirTemp, 'RT'));
    fs.writeFileSync(path.join(dirTemp, 'RT', 'foto_0.jpg'), 'a');

    qaSyndi.moverParaSubpastaSyndi(dirTemp, 'foto_0.jpg', 'RT');
    assert.equal(fs.existsSync(path.join(dirTemp, 'RT')), false);
    assert.equal(fs.existsSync(path.join(dirTemp, 'foto_0.jpg')), true);
});

test('moverParaSubpastaSyndi nao remove subpasta se ainda tiver arquivo', () => {
    const dirTemp = criarDirTemp();
    fs.mkdirSync(path.join(dirTemp, 'RT'));
    fs.writeFileSync(path.join(dirTemp, 'RT', 'foto_0.jpg'), 'a');
    fs.writeFileSync(path.join(dirTemp, 'RT', 'foto_1.jpg'), 'b');

    qaSyndi.moverParaSubpastaSyndi(dirTemp, 'foto_0.jpg', 'RT');
    assert.equal(fs.existsSync(path.join(dirTemp, 'RT')), true);
    assert.equal(fs.existsSync(path.join(dirTemp, 'RT', 'foto_1.jpg')), true);
});

test('deletarFotoSyndi remove arquivo de foto', () => {
    const dirTemp = criarDirTemp();
    fs.writeFileSync(path.join(dirTemp, 'foto_0.jpg'), 'a');

    qaSyndi.deletarFotoSyndi(dirTemp, 'foto_0.jpg');
    assert.equal(fs.existsSync(path.join(dirTemp, 'foto_0.jpg')), false);
});

test('deletarFotoSyndi lanca erro se arquivo nao existe', () => {
    const dirTemp = criarDirTemp();
    assert.throws(() => qaSyndi.deletarFotoSyndi(dirTemp, 'nao-existe.jpg'), /Arquivo nao encontrado/);
});

test('deletarFotoSyndi bloqueia path traversal', () => {
    const dirTemp = criarDirTemp();
    assert.throws(() => qaSyndi.deletarFotoSyndi(dirTemp, '../../etc/passwd'), /Arquivo nao encontrado/);
});

test('lerDestinosOs le mapa do arquivo _destinos_qa.txt', () => {
    const dirTemp = criarDirTemp();
    fs.writeFileSync(qaSyndi.caminhoDestinosOs(dirTemp), '7898133020049=Mockup\n7898133020050=Recorte\n');

    const mapa = qaSyndi.lerDestinosOs(dirTemp);
    assert.equal(mapa['7898133020049'], 'Mockup');
    assert.equal(mapa['7898133020050'], 'Recorte');
});

test('lerDestinosOs devolve mapa vazio se arquivo nao existe', () => {
    const dirTemp = criarDirTemp();
    const mapa = qaSyndi.lerDestinosOs(dirTemp);
    assert.deepEqual(mapa, {});
});

test('escreverDestinosOs grava mapa no arquivo _destinos_qa.txt', () => {
    const dirTemp = criarDirTemp();
    const mapa = { '7898133020049': 'Mockup', '7898133020050': 'Recorte' };

    qaSyndi.escreverDestinosOs(dirTemp, mapa);
    const conteudo = fs.readFileSync(qaSyndi.caminhoDestinosOs(dirTemp), 'utf8');
    assert.match(conteudo, /7898133020049=Mockup/);
    assert.match(conteudo, /7898133020050=Recorte/);
});

test('escreverDestinosOs apaga arquivo se mapa ficar vazio', () => {
    const dirTemp = criarDirTemp();
    fs.writeFileSync(qaSyndi.caminhoDestinosOs(dirTemp), 'teste');

    qaSyndi.escreverDestinosOs(dirTemp, {});
    assert.equal(fs.existsSync(qaSyndi.caminhoDestinosOs(dirTemp)), false);
});

test('obterDestinoGtin le destino de um GTIN especifico do arquivo', () => {
    const dirTemp = criarDirTemp();
    fs.writeFileSync(qaSyndi.caminhoDestinosOs(dirTemp), '7898133020049=Recorte\n');

    const destino = qaSyndi.obterDestinoGtin(dirTemp, '7898133020049');
    assert.equal(destino, 'Recorte');
});

test('obterDestinoGtin devolve null se GTIN nao tem destino', () => {
    const dirTemp = criarDirTemp();
    const destino = qaSyndi.obterDestinoGtin(dirTemp, '7898133020049');
    assert.equal(destino, null);
});

test('removerDestinoGtin remove entrada de um GTIN do arquivo', () => {
    const dirTemp = criarDirTemp();
    fs.writeFileSync(qaSyndi.caminhoDestinosOs(dirTemp), '7898133020049=Mockup\n7898133020050=Recorte\n');

    qaSyndi.removerDestinoGtin(dirTemp, '7898133020049');
    assert.equal(qaSyndi.obterDestinoGtin(dirTemp, '7898133020049'), null);
    assert.equal(qaSyndi.obterDestinoGtin(dirTemp, '7898133020050'), 'Recorte');
});

test('removerDestinoGtin apaga arquivo se fica vazio apos remover', () => {
    const dirTemp = criarDirTemp();
    fs.writeFileSync(qaSyndi.caminhoDestinosOs(dirTemp), '7898133020049=Mockup\n');
...
test('verificarEOrganizarOsNone reorganiza do legadoDestinoDir pra pasta da OS em agConferenciaDir', async () => {
    ...
    fs.mkdirSync(path.join(agConferenciaDir, 'OS_555', '7891234567890'), { recursive: true });
...
test('pescarGtins aceita GTIN direto na raiz da origem, misturado com GTIN dentro de pasta de mes', async () => {
    const dirTemp = criarDirTemp();
    const origem = path.join(dirTemp, 'Origem');
    const destino = path.join(dirTemp, 'Destino');

    // GTIN direto na raiz, sem pasta de mes por cima
    fs.mkdirSync(path.join(origem, '3333333333333'), { recursive: true });
    fs.writeFileSync(path.join(origem, '3333333333333', 'foto.jpg'), 'c');
    // GTIN dentro de pasta de mes, no mesmo drive
    fs.mkdirSync(path.join(origem, 'Outubro', '4444444444444'), { recursive: true });
    fs.writeFileSync(path.join(origem, 'Outubro', '4444444444444', 'foto.jpg'), 'd');

    const resultado = await qaSyndi.pescarGtins(origem, destino);

    assert.deepEqual(resultado.novos.sort(), ['3333333333333', '4444444444444']);
    assert.equal(resultado.erros.length, 0);
    assert.ok(fs.existsSync(path.join(destino, '3333333333333', 'foto.jpg')));
    assert.ok(fs.existsSync(path.join(destino, '4444444444444', 'foto.jpg')));

    const mapa = qaSyndi.lerControleLegado(destino);
    assert.equal(mapa['3333333333333'].mesOrigem, null);
    assert.equal(mapa['4444444444444'].mesOrigem, 'Outubro');
});
