...
async function pescarGtinsComEstado(origemDir, destinoDir, opcoes = {}) {
    ...
    try {
        ...
        // Estrategia: varrer TODA a arvore da origem e extrair TODOS os GTINs, independente
        // de estarem em subpastas de mes ou raiz. Copia tudo solto no destino.
        if (!estadoAnterior || !estadoAnterior.proximos || Object.keys(estadoAnterior.proximos).length === 0) {
            const todosOsGtins = new Set();

            function varrerProfundamente(pastaAtual, profundidade = 0) {
                if (profundidade > 5) return; // Limita profundidade pra nao ficar infinito
                try {
                    const itens = fs.readdirSync(pastaAtual, { withFileTypes: true });
                    for (const item of itens) {
                        if (item.isDirectory()) {
                            const nomePasta = item.name;
                            // Se o nome parece um GTIN, adiciona
                            if (REGEX_PASTA_GTIN.test(nomePasta)) {
                                todosOsGtins.add(nomePasta);
                            } else {
                                // Continua varrendo pra encontrar GTINs dentro de subpastas
                                const caminhoSubpasta = path.join(pastaAtual, nomePasta);
                                varrerProfundamente(caminhoSubpasta, profundidade + 1);
                            }
                        }
                    }
                } catch (err) {
                    // Ignora erros de leitura de pastas
                }
            }

            varrerProfundamente(origemDir);

            if (todosOsGtins.size > 0) {
                estado.proximos['todos'] = Array.from(todosOsGtins);
            }
        }
        ...
        // Processa todos os GTINs (foram encontrados em qualquer lugar da origem)
        const grupoGtins = estado.proximos['todos'] || [];

        if (!estado.processados['todos']) {
            estado.processados['todos'] = [];
        }

        for (const pastaGtinNome of grupoGtins) {
            processados++;
            ...
            if (estado.processados['todos'].includes(pastaGtinNome)) {
            ...
            // Procura o GTIN em qualquer lugar da origem
            let originemPath = null;
            if (fs.existsSync(path.join(origemDir, pastaGtinNome))) {
                originemPath = path.join(origemDir, pastaGtinNome);
            } else {
                // Procura dentro de subpastas
                for (const subpasta of entradasRaiz) {
                    const caminhoPossivel = path.join(origemDir, subpasta, pastaGtinNome);
                    if (fs.existsSync(caminhoPossivel)) {
                        originemPath = caminhoPossivel;
                        break;
                    }
                }
            }
            ...
            if (!originemPath) {
                resultado.erros.push('GTIN ' + gtin + ': nao encontrado na origem');
                onCallback({
                    tipo: 'erro',
                    gtin,
                    mes: null,
                    msg: 'nao encontrado na origem',
                    processados,
                    total: totalGtins
                });
                continue;
            ...
            try {
                ...
                // Verifica se GTIN ja esta no JSON (legado, pendente ou finalizado)
                if (controleLegado[gtin]) {
                    resultado.jaExistiam.push(gtin);
                    estado.processados['todos'].push(pastaGtinNome);
                    onCallback({
                        tipo: 'ja-existe',
                        gtin,
                        status: controleLegado[gtin].status,
                        mes: null,
                        processados,
                        total: totalGtins
                    });
                ...
                resultado.erros.push('GTIN ' + gtin + ': ' + err.message);
                onCallback({
                    tipo: 'erro',
                    gtin,
                    mes: null,
        ...
        // Remove todos os GTINs da fila de proximos
        delete estado.proximos['todos'];
