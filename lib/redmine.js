...
// Grava Responsavel/Quantidades/Responsaveis QA na issue aberta do GTIN, num PUT so. Devolve
// { gravado: false } sem tocar na rede se NENHUM dos campos aceitos por montarCamposEdicao
// vier preenchido (userId nao e um desses campos - so serve pro bloqueio de identidade em
// POST /api/aprovar, fora desta funcao). Lanca erro se nao achar issue aberta ou o PUT
// falhar - quem chama (POST /api/aprovar) BLOQUEIA o aprovar nesse caso, diferente do
// retrabalho.
...
// Busca a issue aberta do GTIN com os 6 campos da aba "QA para Edicao" (Situacao +
// Responsavel + Quantidades + Responsavel QA Imagem + Responsavel 3 Check) - so leitura,
// usado pra pre-preencher a tela antes do analista editar. issue:null se nao achar ficha
// aberta (front mostra aviso, igual sphoto).
...
// Grava Situacao/Responsavel/Quantidades/Responsavel QA Imagem/Responsavel 3 Check na issue
// aberta do GTIN, num PUT so - usada pela aba "QA para Edicao", independente do fluxo do
// Aprovar (que usa gravarCamposEdicao e nunca grava Situacao). userId NAO entra mais nesta
// funcao (removido deliberadamente - cf_85 virou um dropdown manual comum nesta aba, ver
// docs/superpowers/specs/2026-07-28-syndi-qa-campos-edicao-3check-design.md), entao
// { gravado: false } volta a ser um caminho real: acontece sempre que o analista clica
// Gravar com os 6 campos vazios. Lanca erro se nao achar issue aberta ou o PUT falhar.
...
// Atribui fotografo "Flavio Demarchi" (217) a um GTIN quando reorganiza OS_NONE.
// Nao lanca erro se nao achar issue aberta - ja esta registrada em log pela verificacao.
const CF_FOTOGRAFO = 62;
const FLAVIO_DEMARCHI_ID = 217;

async function gravarFotografoOsNone(basePath, gtin) {
    try {
        const issue = await buscarIssueAbertaPorGtin(basePath, gtin);
        if (!issue) return { gravado: false, motivo: 'Issue nao encontrada' };

        const resp = await redmineFetch(basePath, '/issues/' + issue.id + '.json', {
            method: 'PUT',
            body: JSON.stringify({ issue: { custom_fields: [{ id: CF_FOTOGRAFO, value: String(FLAVIO_DEMARCHI_ID) }] } })
        });
        if (!resp.ok) {
            const texto = await resp.text();
            return { gravado: false, motivo: 'Redmine ' + resp.status + ': ' + texto };
        }
        return { gravado: true, issueId: issue.id };
    } catch (err) {
...
// Busca a OS do GTIN no Redmine (campo customizado ou subject)
async function buscarOsDoGtin(basePath, gtin) {
    try {
        const issue = await buscarIssueAbertaPorGtin(basePath, gtin);
        if (!issue) return { os: null, motivo: 'Issue nao encontrada' };

        // Extrai OS do subject (geralmente começa com OS_123 ou vem no formato "OS_123 - Descrição")
        const subject = issue.subject || '';
        const matchOS = subject.match(/OS[_\s]*(\d+)/i);
        if (matchOS) {
            return { os: matchOS[1], issueId: issue.id };
        }

        // Se nao encontrar no subject, procura em campo customizado se houver
        if (issue.custom_fields && Array.isArray(issue.custom_fields)) {
            for (const field of issue.custom_fields) {
                if (field.name && field.name.includes('OS')) {
                    const osMatch = String(field.value).match(/(\d+)/);
                    if (osMatch) {
                        return { os: osMatch[1], issueId: issue.id };
                    }
                }
            }
        }

        return { os: null, motivo: 'OS nao encontrada na issue' };
    } catch (err) {
        return { os: null, motivo: err.message };
    }
}
