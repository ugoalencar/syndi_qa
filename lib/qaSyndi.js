...
// Lista todos os GTINs em OS_NONE - fotos do fotografo externo que ainda nao tem
// OS atribuida. Usa a mesma logica de listarFila mas nao procura por prefixo OS_,
// so varre os arquivos de imagem na pasta OS_NONE e suas subpastas (RT/IS/AP).
...
function verificarEOrganizarOsNone(agConferenciaDir, cadastroOcrDir) {
    if (!cadastroOcrDir) {
    ...
    const osNoneData = listarOsNone(agConferenciaDir);

    // Varre cada GTIN em OS_NONE
    for (const item of osNoneData) {
        const gtin = item.gtin;
        const pastaGtinNome = item.pastaGtinNome;
        const pastaOsNonePath = path.join(agConferenciaDir, 'OS_NONE', pastaGtinNome);

        // Valida as marcas OCR
        const validacao = validarMarcasOcrParaGtin(pastaOsNonePath, 2);

        if (!validacao.valido) {
            resultado.avisos.push('GTIN ' + gtin + ': ' + validacao.mensagem);
            continue;
        }

        // Encontra a OS de destino para este GTIN
        const osInfo = localizarGtinEmAgConferencia(agConferenciaDir, gtin);
        ...
        // Move os arquivos OCR marcados para C:\Cadastro\OCR
        const arquivosOcr = validacao.arquivos;
        let movidosEsteGtin = 0;

        for (const nomeArquivo of arquivosOcr) {
            try {
                // Resolve o caminho relativo (pode estar em subpasta RT/IS/AP)
                // O nomeArquivo pode ser "foto.jpg" (raiz) ou "RT/foto.jpg" (subpasta)
                let caminhoOrigem = null;
                let nomeBaseArquivo = nomeArquivo; // Nome sem caminho (para o destino)

                // Se tem barra, e um caminho com subpasta
                if (nomeArquivo.includes('/') || nomeArquivo.includes('\\')) {
                    const partes = nomeArquivo.split(/[/\\]/);
                    nomeBaseArquivo = partes[partes.length - 1]; // Ultima parte = nome do arquivo
                ...
                // Nome do arquivo de destino: gtin_nomebase (sem o prefixo de subpasta)
                const nomeDestino = gtin + '_' + nomeBaseArquivo;
                const caminhoDestino = path.join(cadastroOcrDir, nomeDestino);

                // Copia o arquivo (nao move, pra manter o original em OS_NONE)
                fs.copyFileSync(caminhoOrigem, caminhoDestino);

                resultado.movidos.push('GTIN ' + gtin + '/' + nomeArquivo + ' -> ' + caminhoDestino);
                movidosEsteGtin++;
            } catch (err) {
                resultado.erros.push('GTIN ' + gtin + '/' + nomeArquivo + ': ' + err.message);
            }
        }

        if (movidosEsteGtin > 0) {
            resultado.movidos.unshift('GTIN ' + gtin + ' (OS ' + osInfo.os + '): ' + movidosEsteGtin + ' arquivo(s) processado(s)');
        }
    }
