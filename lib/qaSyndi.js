...
async function verificarEOrganizarOsNone(agConferenciaDir, cadastroOcrDir, opcoes) {
    if (!cadastroOcrDir) {
        cadastroOcrDir = 'C:\\Cadastro\\OCR';
    }
    opcoes = opcoes || {};
    ...
    for (const item of osNoneData) {
        ...
        try {
            ...
            const copiarDiretorioRecursivo = (src, dst) => {
            ...
            for (const nomeArquivo of arquivosOcr) {
                ...
                resultado.movidos.push('  OCR: ' + nomeArquivo + ' -> C:\\Cadastro\\OCR\\' + nomeDestino);
            }
