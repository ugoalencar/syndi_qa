...
function inferirCamposEdicao(pastaGtinPath, destinoOs) {
    ...
    if (destinoOs === 'Recorte') {
        ...
        if (comCoding.length === 0) {
            return { destino: 'indefinido', motivo: 'Subpasta Recorte marcada mas nenhuma foto _coding - responsavel nao decidido automaticamente (quantidade ja preenchida abaixo)', campos };
        }
...
function gravarTxtRecorte(pastaGtinPath, gtin, orientacoes) {
    let conteudo = `GTIN: ${semQuebraDeLinha(gtin)}\n`;
    if (orientacoes && orientacoes.length) {
        conteudo += 'Orientacoes:\n' + orientacoes.map(o => `- ${semQuebraDeLinha(o)}`).join('\n') + '\n';
    }
    const caminhoTxt = path.join(pastaGtinPath, `Recorte_${gtin}.txt`);
    fs.writeFileSync(caminhoTxt, conteudo, 'utf8');
    return caminhoTxt;
}
...
function aprovarGtin(agConferenciaDir, agEnvioDir, pastaOsNome, pastaGtinNome, gtin, mockupInfo, recorteInfo) {
    ...
    if (mockupInfo) {
        // gtin puro (nao pastaGtinNome) - pastaGtinNome e o nome decorado da pasta no disco,
        // que pode um dia ganhar sufixo (ver REGEX_PASTA_GTIN); usar ele aqui faria o TXT
        // herdar essa decoracao no nome do arquivo e no conteudo.
        gravarTxtMockup(origem, mockupInfo.gtin, mockupInfo.numero, mockupInfo.orientacoes);
    }
    if (recorteInfo && recorteInfo.orientacoes && recorteInfo.orientacoes.length) {
        gravarTxtRecorte(origem, recorteInfo.gtin, recorteInfo.orientacoes);
    }
